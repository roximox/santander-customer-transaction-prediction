"""Run the registered M03-FS-001 training-only L1 Feature Selection experiment."""
...
COMPARISON_CSV = PROJECT_ROOT / "reports/tables/feature_selection_pca_comparison.csv"
COMPARISON_JSON = PROJECT_ROOT / "reports/tables/feature_selection_pca_comparison.json"
CV_FIGURE = PROJECT_ROOT / "reports/figures/feature_selection_cv_scores.pdf"


def _requested_outputs() -> list[Path]:
    return [
        EXPERIMENTS_DIR / f"{FS_EXPERIMENT_ID}_fold_results.csv",
        EXPERIMENTS_DIR / f"{FS_EXPERIMENT_ID}_summary.json",
        CV_FIGURE,
    ]


def refuse_existing_outputs() -> None:
    """Refuse duplicate FS artifacts without touching other experiment results."""
    existing = [
    ...
    if REGISTRY_PATH.exists():
        ...
        if FS_EXPERIMENT_ID in registry["experiment_id"].astype(str).tolist():
            existing.append(f"registry:{FS_EXPERIMENT_ID}")
    if existing:
        details = "\n- ".join(existing)
        raise FileExistsError(
            f"{FS_EXPERIMENT_ID} outputs already exist and were not overwritten. "
...
def main() -> None:
    """Execute, register, and plot the L1 Feature Selection baseline."""
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        fold_results, summary = run_and_save_experiment(
            ...
            X_train,
            y_train,
            experiment_id=FS_EXPERIMENT_ID,
    ...
    save_fs_cv_figure(fold_results, CV_FIGURE)

    metric_rows = {name: values for name, values in summary["metrics"].items()}
    print(f"{FS_EXPERIMENT_ID} mean and standard deviation by metric:")
