"""Run the single registered M04-HGB-001 training-only scientific baseline."""
...
from src.gradient_boosting import (  # noqa: E402
...
def refuse_existing_outputs() -> None:
    ...
    outputs = (
        EXPERIMENTS_DIR / f"{HIST_GRADIENT_BOOSTING_EXPERIMENT_ID}_fold_results.csv",
        EXPERIMENTS_DIR / f"{HIST_GRADIENT_BOOSTING_EXPERIMENT_ID}_summary.json",
    )
    ...
    if REGISTRY_PATH.exists():
        ...
        if HIST_GRADIENT_BOOSTING_EXPERIMENT_ID in registry["experiment_id"].astype(str).tolist():
            existing.append(f"registry:{HIST_GRADIENT_BOOSTING_EXPERIMENT_ID}")
    if existing:
        raise FileExistsError(
            f"{HIST_GRADIENT_BOOSTING_EXPERIMENT_ID} outputs already exist and were not overwritten. "
...
def print_result_summary(fold_results: pd.DataFrame, summary: dict[str, object]) -> None:
    ...
    print(f"{HIST_GRADIENT_BOOSTING_EXPERIMENT_ID} validation summary:")
...
def main() -> None:
    """Evaluate and register the exact HistGradientBoosting baseline on training data."""
    ...
    classifier = create_hist_gradient_boosting_baseline()
    fold_results, summary = run_and_save_experiment(
        classifier,
        X_train,
        y_train,
        experiment_id=HIST_GRADIENT_BOOSTING_EXPERIMENT_ID,
        model_name=HIST_GRADIENT_BOOSTING_MODEL_NAME,
        member="Member 04",
